...
**Объём:** ре-скин фронтенда под уровень современных LLM-чатов (гибрид: структура ChatGPT — sidebar + узкая колонка, читаемость Claude — тёплая палитра, типографика) без изменения продуктовой логики (hooks/reducer нетронуты). Дизайн-токены + тёмная тема (`prefers-color-scheme` + переключатель); injection-safe markdown-рендеринг ответов (без `dangerouslySetInnerHTML`, совместим с маркерами [n]); стоп-генерация, копирование ответа, регенерация, авто-скролл. Изменение CSS-стека (Tailwind) — только через ADR до подключения.

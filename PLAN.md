...
Фазы **0–7 — MVP-трек** , **P0–P4 — production-трек**. Трудозатраты — оценка для одного разработчика с Claude Code (календарные дни неполной занятости).
...
**Зависимости:** фаза 0 · **Оценка:** 3–4 дня
...
**Зависимости:** фаза 1 · **Оценка:** 5–7 дней
...
**Зависимости:** фаза 2 · **Оценка:** 4–5 дней
...
**Зависимости:** фаза 3 · **Оценка:** 5–7 дней
...
**Зависимости:** фаза 4 · **Оценка:** 5–6 дней
...
**Зависимости:** фаза 5 (нужен полный пайплайн) · **Оценка:** 4–6 дней

### Фаза 7 — Демо-упаковка и hardening
**Цель:** чистый запуск с нуля, наполненное демо, отработанные сценарии.
...
**Зависимости:** фаза 6 · **Оценка:** 3–4 дня

**Итого MVP: ~31–42 дня.** Линия демонстрации: после фазы 7 показывается продукт (чат с цитатами и честными отказами), инженерия (ADR, тесты, CI, наблюдаемость) и качество (eval-отчёты).

---

## Production-трек

Каждая P-фаза перед стартом получает детальную проработку по образцу MVP-фаз; для P3 и P4 сначала фиксируются новые ADR (план миграции vector store, топология K8s, граф знаний, агентная топология).

### P0 — UI-редизайн: современный чат-интерфейс ✅
**Объём:** ре-скин фронтенда под уровень современных LLM-чатов (гибрид: структура ChatGPT — sidebar + узкая колонка, читаемость Claude — тёплая палитра, типографика) без изменения продуктовой логики (hooks/reducer нетронуты). Дизайн-токены + тёмная тема (`prefers-color-scheme` + переключатель); injection-safe markdown-рендеринг ответов (без `dangerouslySetInnerHTML`, совместим с маркерами [n]); стоп-генерация, копирование ответа, регенерация, авто-скролл. CSS-стек — Tailwind 4 по [ADR-011](docs/adr/ADR-011-frontend-css-stack.md).
**DoD:** сценарий [docs/demo-script.md](docs/demo-script.md) проходит в обеих темах; «честные» элементы (цитаты [n] с поповерами, confidence, degraded, refusal + nearest_documents) видимы во всех состояниях; Vitest зелёный без изменения проверяемого поведения; скриншоты `docs/img/` обновлены. **Оценка:** 1–2 недели. **Зависимости:** фаза 7.
**Статус:** выполнено — 5 под-фаз в main (ADR-011, токены+shell+логин, чат, markdown, действия чата); Vitest 28 зелёных.
...
flowchart LR
    ...
    F7 --> P0[P0 UI-редизайн] --> P1[P1 Доступ] --> P2[P2 Мультитенант+источники]

...
5. **Раннер**: CLI `python -m lyra.evals run --dataset golden [--judge cloud|local]` + Makefile-цель `make eval` (обёртка CLI с дефолтами) — прогон каждого item через настоящий граф (temperature=0), запись eval_runs/eval_records + `config_snapshot` (git_ref, версии промптов, модели, retrieval-параметры, chunking_config), JSON+markdown отчёт с worst items; Celery-задача (очередь evals) + `POST/GET /admin/eval-runs` по api-contract §6; сравнение с baseline (последний успешный run на main — файл-артефакт `evals/baseline.json`).

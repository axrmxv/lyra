...
Ты работаешь над проектом **LYRA** — RAG-платформой корпоративных знаний (FastAPI + LangGraph + PostgreSQL/pgvector + Redis + Celery + React/Vite/TS, всё локально в docker-compose, LLM — Ollama). Прочитай перед началом: `docs/architecture.md`, `docs/nfr.md` (§5, §6), `PLAN.md` (фаза 0).
...
5. **Frontend:** Vite-болванка с одной страницей «LYRA», прокси `/api` на backend, eslint+prettier+vitest настроены, один smoke-тест рендера.
